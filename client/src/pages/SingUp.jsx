...
function SingUp() {
    ...
    useEffect(()=>{
        const sendData=async ()=>{
            try{
                const result=await axios.post('/api/signUpUser',{name:nm,email:em,password:pw},{headers:{"Content-Type":"application/x-www-form-urlencoded"}})
                console.log('SignUp Succesful')
                setEr('SignUp Succesful.Redirecting')
                setTimeout(()=>{
                    Navigate('/')
                },3000)
            }catch(e){
                console.log(e.response.status)
                setEr('Error Signing Up')
    ...
    function validateForm(){
        ...
        if(pw.length<8){
            setEr('Password too short and weak')
            flag=1
        }
        if(!em.includes('@')){
            setEr('Incorrect Email')
            flag=1
        }
        if (flag==0){
            setSend(true)
  ...
  return (
    <div className=" bg-yellow-50 w-[100vw] h-[100vh] relative flex flex-col items-center justify-center text-lg p-4 gap-y-4">
        ...
        <div className='flex flex-col gap-y-4'>
            <input type="text" className='rounded-md h-[7vh] drop-shadow-md px-2 outline-none text-[15px]'placeholder='Name' value={nm} onChange={(e)=>{setNm(e.target.value)}}/>
            ...
            <button onClick={validateForm} className='rounded-md h-[7vh] drop-shadow-md bg-green-500 text-[15px] jost-600 px-2 text-white'>SignUp</button>
        ...
        <Link to='/login' className='text-[11px] hover:text-blue-600'>Already have an account?Login here..</Link>
        <Link to='/'>
